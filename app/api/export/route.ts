...
export async function GET(request:NextRequest){
  ...
  const month=request.nextUrl.searchParams.get("month")??undefined;const accountId=request.nextUrl.searchParams.get("account")??undefined;const encoder=new TextEncoder();
  const stream=new ReadableStream({async start(controller){try{controller.enqueue(encoder.encode(`${header.map(csv).join(",")}\r\n`));let page=1;while(true){const batch=await getTransactionsPage({month,accountId,page,pageSize:500});for(const row of batch.items){controller.enqueue(encoder.encode(`${[row.transaction_date,row.type,row.description,row.accounts?.name,row.expense_categories?.name??row.income_sources?.name,row.original_amount,row.original_currency,row.amount_pkr,row.amount_usd].map(csv).join(",")}\r\n`))}if(page>=batch.totalPages)break;page++}controller.close()}catch(error){controller.error(error)}}});
