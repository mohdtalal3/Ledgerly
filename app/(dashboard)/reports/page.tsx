...
export default async function Reports({searchParams}:{searchParams:Promise<{month?:string;page?:string}>}){
  const q=await searchParams;const month=q.month??new Date().toISOString().slice(0,7);const[summary,expenses]=await Promise.all([getPeriodSummary(month),getTransactionsPage({type:"expense",month,page:Math.max(1,Number(q.page)||1)})]);
  return <><header className="page-header"><div><div className="eyebrow">Analytics & export</div><h1>Reports</h1><p className="muted">Patterns you can act on, with raw data when you need it.</p></div><Link className="btn" href={`/api/export?month=${month}`}><Download size={17}/>Export CSV</Link></header>
    <div className="toolbar"><form><input name="month" type="month" defaultValue={month}/><button className="btn">View report</button></form></div>
