...
export default async function Expenses({searchParams}:{searchParams:Promise<{month?:string;search?:string;account?:string;page?:string}>}){
  const q=await searchParams;const month=q.month??new Date().toISOString().slice(0,7);const page=Math.max(1,Number(q.page)||1);const[ledger,summary,accounts]=await Promise.all([getTransactionsPage({type:"expense",month,search:q.search,accountId:q.account,page}),getPeriodSummary(month),getAccounts(true)]);
  return <><header className="page-header"><div><div className="eyebrow">Money out</div><h1>Expenses</h1><p className="muted">See where your money goes without the noise.</p></div><Link className="btn primary icon" href="/transactions/new"><Plus/></Link></header>
    ...
    <div className="section toolbar"><form><input aria-label="Search expenses" name="search" placeholder="Search expenses…" defaultValue={q.search}/><input aria-label="Month" name="month" type="month" defaultValue={month}/><select name="account" aria-label="Account" defaultValue={q.account??""}><option value="">All accounts</option>{accounts.map(account=><option key={account.id} value={account.id}>{account.name}</option>)}</select><button className="btn">Filter</button></form></div>
    <TransactionList transactions={ledger.items} editable editReturnTo="/expenses"/><Pagination page={ledger.page} total={ledger.count} pathname="/expenses" query={{month,search:q.search,account:q.account}}/>
  </>
}
